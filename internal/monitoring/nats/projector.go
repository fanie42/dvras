...
func New(
    ...
    // subStarted, err := conn.Subscribe(
    //     "dvras.devices.started",
    //     app.onStarted,
    // )

    // subStopped, err := conn.Subscribe(
    //     "dvras.devices.stopped",
    //     app.onStopped,
    // )

    // subDatapointAcquired, err := conn.Subscribe(
    //     "dvras.datapoints.acquired",
    //     app.onDatapointAcquired,
    // )

    return eb
}

// These should be closer to the domain. It will be exactly the same for any
// projection from the events -> Make the handlers per event.
